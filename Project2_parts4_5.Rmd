...
mu = the level of differential gene expression between electric organ and skeletal muscle.
H_0: mu = 0
H_1: mu != 0
...
campys = DESeqDataSetFromHTSeqCount(campy_meta, directory = "counts_tar", design=~SpeciesSmaller+Tissue)
```

I chose to use SpeciesSmaller+Tissue because Tissue is the variable that we want to test for, and the species is another variable that could account for variation that we want to control for. I also added SpeciesSmaller:Tissue because when working with Hannah B, we decided that those 2 could potentially have an impact on each other's genetic expression, so we wanted to account for that in the design formula.
...
I chose cpm beacuse it normalizes the data before filtering, which helps account for sequencing depth.
